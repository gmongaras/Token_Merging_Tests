    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not hasattr(self, "key_merge"):
            self.key_merge = BipartiteSoftMatching(key, 1000)
        key = self.key_merge.merge(key, mode="mean")
        value = self.key_merge.merge(value, mode="mean")
        if not hasattr(self, "query_merge"):
            self.query_merge = BipartiteSoftMatching(query, 1024)#query.shape[1]//8)
        # query = self.query_merge.merge(query, mode="MLERP")
