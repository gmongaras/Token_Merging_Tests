    ...
    def forward(
        ...
        if self.merge_attn:
            ...
            if not hasattr(self, "hidden_states_merge"):
                ...
                self.hidden_states_merge = BipartiteSoftMatching(norm_hidden_states, int(norm_hidden_states.shape[1]*self.merge_per), generator=generator, method=self.merge_method)
        ...
        if self.merge_ff:
            ...
            if not hasattr(self, "hidden_states_merge_ff"):
                generator = torch.Generator()#.manual_seed(42)
                self.hidden_states_merge_ff = BipartiteSoftMatching(norm_hidden_states, int(norm_hidden_states.shape[1]*self.merge_per), generator=generator, method=self.merge_method)

            # Merge the hidden states
            norm_hidden_states = self.hidden_states_merge_ff.merge(norm_hidden_states, mode="mean")
        ...
        # Unmerge
        if self.merge_ff:
            ff_output = self.hidden_states_merge.unmerge(ff_output)
