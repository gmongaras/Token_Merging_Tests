...
class CustomProcessor:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        def calculate_indices(tensor, num):
        ...
        # Calculate indices
        if self.idx > 2 and self.idx < 20:
            # Check if self.key_merge exists
            if not hasattr(self, "key_merge"):
                self.key_merge = BipartiteSoftMatching(key, 2000)
                key_idx, key_idx_flat, key_idx_unmerged = calculate_indices(key, 2000)
            ...
            if not hasattr(self, "query_merge"):
                self.query_merge = BipartiteSoftMatching(query, 100)
                query_idx, query_idx_flat, query_idx_unmerged = calculate_indices(query, 100)
